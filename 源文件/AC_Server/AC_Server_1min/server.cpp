...
Server::Server(int serverId, int mode,int defaultTargetTemp,int defaultFanSpeed,float feeRate,float fan[],QWidget *parent) : QWidget(parent)
{
    ...
    connect(durationTimer,&QTimer::timeout,[=](){
        emit signalDurationRefresh();
        duration++;
    });

    tempTimer=new QTimer(this);
    tempTimer->setInterval(60000);
    connect(tempTimer,&QTimer::timeout,[=](){
        if(mode==SERVER_MODE_WARM)
        {
            if(currentTemp<targetTemp)
            {
                if(fanSpeed==0)
                {
                    this->currentTemp+=fan[0];
                    GetFee(fan[0]);
                }
                else if(fanSpeed==1)
                {
                    this->currentTemp+=fan[1];
                    GetFee(fan[1]);
                }
                else if(fanSpeed==2)
                {
                    this->currentTemp+=fan[2];
                    GetFee(fan[2]);
                }
                emit signalSendInfo();
            }
...
bool Server::stopServe()
{
    if(tempTimer->isActive())
    {
        // 认为此次温度已经到达，只是有5s的误差，在关闭后，手动变化温度
        if(tempTimer->remainingTime()<=10000)
        {
            tempTimer->stop();
            durationTimer->stop();
            state=SERVER_STATE_FREE;
...
//设定Server信息
void Server::SetServer(int roomId, int originTemp, int targetTemp,float currentTemp,int fanSpeed,float fee,int duration)
{
    this->roomId = roomId;

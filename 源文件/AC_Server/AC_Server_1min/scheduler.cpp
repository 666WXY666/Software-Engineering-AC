...
void Scheduler::init(int mode,int defaultTemp,int defaultFanSpeed,float feeRate,float fan[3],int tempLowLimit,int tempHighLimit)
{
    ...
    for (int i=0;i<3;i++)
    {
        ...
        connect(serverList[i], &Server::signalSendInfo,[=](){
            // 刷新当前温度，总费用
            allRoomMap[serverList[i]->roomId]->currentTemp=serverList[i]->currentTemp;
            allRoomMap[serverList[i]->roomId]->fee=serverList[i]->fee;

            QJsonObject requestInfoJson;
            requestInfoJson.insert("request",REQUEST_INFO);
            QJsonObject requestInfoDataJson;
            requestInfoDataJson.insert("TotalFee",serverList[i]->fee);
            requestInfoDataJson.insert("CurrentTemp",serverList[i]->currentTemp);
        ...
        connect(serverList[i], &Server::signalDurationRefresh,[=](){
...
void Scheduler::sync(int roomId)// 当房间从服务队列离开前，同步房间与服务对象信息(当前温度，费用，使用时长)，且将等待服务时长设为0
{
    ...
    allRoomMap[roomId]->currentTemp=serverList[allRoomMap[roomId]->serverId]->currentTemp;
    ...
    allRoomMap[roomId]->fee=serverList[allRoomMap[roomId]->serverId]->fee;
...
int Scheduler::GetReplaceRoomId(int fanSpeed)
{
    ...
    if(lowerFanSpeedRoomIdList.size()!=0)
    {
        ...
        else
        {
            ...
            {
                int longestServeTime=allRoomMap[lowestFanSpeedRoomIdList[0]]->serverTime;
...
bool Scheduler::ChangeFanSpeed(int roomId,int fanSpeed)
{
    ...
    for (QMap<int,Room*>::iterator it=waitQueue->roomMap.begin();it!=waitQueue->roomMap.end();it++)
    {
        if(it.key()==roomId)
        {
            ...
            if(lowerFanSpeedRoomIdList.size()>=1)
            {
                int longestServeTime=allRoomMap[lowerFanSpeedRoomIdList[0]]->serverTime;

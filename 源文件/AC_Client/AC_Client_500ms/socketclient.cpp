...
void SocketClient::JsonAnalyse(QJsonObject receivedJson)
{
    ...
    case REQUEST_SERVE_ON:
    {
        ...
        requestServeOnOkDataJson.insert("CurrentTemp",QString::number(currentTemp, 'f', 2).toFloat());
...
//        tcpSocket->flush();
    ...
    case REQUEST_SERVE_PREEMPT:
    {
        ...
        requestServePreemptOkDataJson.insert("CurrentTemp",QString::number(currentTemp, 'f', 2).toFloat());
...
void SocketClient::SendRewormTemp()// 回温温度心跳包
{
    ...
    requestRewormTempDataJson.insert("CurrentTemp",QString::number(currentTemp, 'f', 2).toFloat());
